import os
from math import degrees, radians, atan2, sqrt, pi, sin, cos, tan
import threading
import time

f = 110 * (2 * sqrt(3))  # the side of the fixed triangle
e = 60 * (2 * sqrt(3))  # the side of the end effector triangle
...
def Forward_Kinematic(t1, t2, t3):
    ...
    x2 = (t + rf * cos(t2)) * cos(pi / 6)
    y2 = x2 * tan(pi / 6)
    z2 = -rf * sin(t2)

    x3 = -(t + rf * cos(t3)) * cos(pi / 6)
    y3 = -x3 * tan(pi / 6)
    z3 = -rf * sin(t3)
...
def write_line_data(begin, end, step):
    ...
    P = [Inverse_Kinematic(x, 0, Pz) for x in x_values]
    ...
    with open(file_path, "w") as file:
        ...
        for theta, x in zip(P, x_values):
            file.write(f"{theta[1]},{theta[2]},{theta[3]},{x},0,{Pz}\n")
...
def print_values():
    for theta1, theta2, theta3, Px, Py, Pz in read_line_data():
        print(
            f"Received values: theta1={theta1}, theta2={theta2}, theta3={theta3}, Px={Px}, Py={Py}, Pz={Pz}"
        )


def write_triangle_data(step):
    Pz = -480
    # Tạo danh sách các giá trị của x1 và y1
    x1_values = list(range(0, 50 + step, step))
    y1_values = []
    ...
    P1 = [Inverse_Kinematic(x, y, Pz) for x, y in zip(x1_values, y1_values)]

    # Tạo danh sách các giá trị của x2 và y2
    x2_values = list(range(50, 0 - step, -step))
    y2_values = []
    ...
    P2 = [Inverse_Kinematic(x, y, Pz) for x, y in zip(x2_values, y2_values)]

    y3_values = list(range(50, -50 - step, -step))

    P3 = [Inverse_Kinematic(0, y, Pz) for y in y3_values]

    file_path = os.path.join("Gui_Delta", "triangle_data.txt")
    # Ghi dữ liệu vào tệp
    with open(file_path, "w") as file:
        # Thêm dòng đầu tiên
        file.write("theta1,theta2,theta3,Px,Py,Pz\n")

        for theta, x, y in zip(P1, x1_values, y1_values):
            file.write(f"{theta[1]},{theta[2]},{theta[3]},{x},{y},{Pz}\n")

        for theta, x, y in zip(P2, x2_values, y2_values):
            file.write(f"{theta[1]},{theta[2]},{theta[3]},{x},{y},{Pz}\n")

        for theta, y in zip(P3, y3_values):
            file.write(f"{theta[1]},{theta[2]},{theta[3]},0,{y},{Pz}\n")
...
def read_triangle_data():
    ...
    file_path = os.path.join("Gui_Delta", "triangle_data.txt")
    with open(file_path, "r") as file:
        # Bỏ qua dàn đầu tiên trong file
        header = next(file)
...
# write_line_data(-100, 100, 10)

# write_triangle_data(5)
# # Tạo các luồng
...
status, P1, P2, P3 = Forward_Kinematic(50, 50, 50)
